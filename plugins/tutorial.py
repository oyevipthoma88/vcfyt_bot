...
SECTIONS = {
    ...
    "boost": (
        ...
        " <b>Live Mic Relay ke saath</b> (naya!): <code>.mic on</code> "
        "se aapki aawaz browser se capture hoti hai, FFmpeg effects "
        "(bass, echo, boost, gain) lagte hain, aur processed audio VC "
        "mein stream hota hai — Telegram ke 200% cap ka bypass!\n"
        "Detail ke liye <b>Live Mic</b> tutorial dekhein."
    ),
    "livemic": (
        "🎤 <b>Live Mic Relay — apni aawaz VC mein bhejein</b>\n\n"
        f"{LINE}\n"
        "<b>Naya system:</b> Ab aapki live aawaz browser se capture hoti hai, "
        "FFmpeg se bass/echo/boost/gain/treble effects lagte hain, aur "
        "processed audio seedha VC mein stream hota hai. Telegram ka 200% "
        "volume cap <b>bypass</b> ho jata hai!\n\n"
        f"{LINE}\n"
        "<b>4 simple steps:</b>\n\n"
        "<b>1.</b> Bot mein login karein (Home → Login → Phone)\n"
        "<b>2.</b> Group mein Voice Chat start karein\n"
        "<b>3.</b> Group mein <code>.mic on</code> bhejein — bot ek link dega\n"
        "<b>4.</b> Link phone mein open karein → Mic ON dabayein → mic permission dein → bolna shuru karein!\n\n"
        f"{LINE}\n"
        "<b>Commands:</b>\n"
        "<code>.mic on</code> — live mic relay start + link generate\n"
        "<code>.mic on -100xxxxxxxx</code> — specific group ke liye\n"
        "<code>.mic off</code> — mic relay stop\n"
        "<code>.mic status</code> — current status\n\n"
        f"{LINE}\n"
        "<b>Settings live mic pe kaise change karein:</b>\n"
        "1. <code>.mic off</code> karein\n"
        "2. Settings set karein: <code>.bass 30</code>, <code>.echo on</code>, <code>.boost 10</code>, <code>.gain 200</code>\n"
        "3. <code>.mic on</code> dobara karein — naye settings ke saath relay start hoga\n\n"
        f"{LINE}\n"
        "<b>Effects jo live mic pe lagte hain:</b>\n"
        "🔊 Volume (0-1000) | 📈 Gain (0-200) | 🎵 Bass (0-100)\n"
        "🎶 Treble (0-100) | 💥 Boost (0-10) | 🔊 Echo (on/off, 0-10)\n\n"
        f"{LINE}\n"
        "<b>Tips:</b>\n"
        "• Link phone mein open karein, mic permission dein\n"
        "• Aawaz kam? <code>.gain 200</code> + <code>.boost 10</code> + <code>.mic on</code>\n"
        "• Echo speech ke liye OFF rakhein\n"
        "• Settings change karne ke liye pehle <code>.mic off</code> zaroori hai\n\n"
        f"{LINE}\n"
        "<b>Problem?</b>\n"
        "Link open nahi ho raha → LIVE_MIC_BASE_URL env check karein (default vcfytbpt22922)\n"
        "Mic permission nahi mili → browser settings mein mic allow karein\n"
        "Aawaz nahi → VC on hai? Link open hai? Mic ON button dabaya?"
    ),
    "vc": (
        " <b>VC Control</b>\n\n"
        f"{LINE}\n"
    ...
    "cmds": (
        ...
        f"{LINE}\n<b>Live</b>\n{LINE}\n"
        ".mic on  .mic off  .mic status  .myboost  /livegain\n\n"
        f"<b>Library</b>\n{LINE}\n"
