...
from helpers.access_control import premium_info
...
async def cmd_start(bot: Client, msg: Message):
    ...
    # Process referral deep link: /start ref_<referrer_id>
    if len(msg.command) > 1 and msg.command[1].startswith("ref_"):
        try:
            referrer_id = int(msg.command[1][4:])
            from plugins.referral import process_referral_start
            await process_referral_start(bot, user.id, referrer_id)
...
async def cmd_mystatus(bot: Client, msg: Message):
    ...
    active_chat_id = next((cid for cid, st in uvc.chats.items() if st.is_playing), None) if uvc else None
    prem = await premium_info(msg.from_user.id)
    await msg.reply_text(status_text(msg.from_user.id, data, uvc, s, prem),
...
async def cb_status(bot, cq):
    ...
    prem = await premium_info(uid)
    await edit_screen(cq.message, status_text(uid, data, uvc, s, prem),
